...
new_d1 <- d %>%
  ...
  summarise(avg_severity = mean(Severity,na.rm=T),avg_temp = mean(Temperature,na.rm = T),avg_wind_chill = mean(Wind_Chill,na.rm = T),
...
a_s <- a_s %>%
  select(NAME,AGE,"X2014","X2015","X2016")%>%
  group_by(NAME)%>%
